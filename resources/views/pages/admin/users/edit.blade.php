        ...
        <form class="w-full max-w-2xl space-y-8 rounded-lg border border-primary/20 bg-white p-10" method="POST"
            ...
            <div>
                ...
                <x-text-input id="nomor_induk" name="nomor_induk" type="text" class="mt-1" required type="number"
                    placeholder="Enter NISN/NIP" :value="old('nomor_induk', $user->nomor_induk)" />
            ...
            <div>
                ...
                <x-text-input id="first_name" name="name" type="text" class="mt-1" required
                    placeholder="Enter first name" :value="old('name', $user->name)" />
            ...
            <div>
                ...
                <select name="jk" id="jk" class="w-full">
                    ...
                    <option value="Laki-Laki" @selected(old('jk', $user->jk) === 'Laki-Laki')>Laki-laki</option>
                    <option value="Perempuan" @selected(old('jk', $user->jk) === 'Perempuan')>Perempuan</option>
                </select>
                ...
                <div class="mt-2 flex flex-col space-y-2">
                    <label class="inline-flex items-center">
                        <input type="radio" name="role" value="admin"
                            class="form-radio text-primary focus:ring-primary" required @checked(old('role', $user->firstRole) === 'admin') />
                        <span class="ml-2">Administrator</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="role" value="teacher"
                            class="form-radio text-primary focus:ring-primary" @checked(old('role', $user->firstRole) === 'teacher') />
                        <span class="ml-2">Teacher</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="role" value="user"
                            class="form-radio text-primary focus:ring-primary" @checked(old('role', $user->firstRole) === 'user') />
